...
async fn main() -> anyhow::Result<()> {
    ...
    let addr: SocketAddr = std::env::var("APP_ADDR")?.parse()?;
    let redis_url = std::env::var("REDIS_URL")?;
    let edge_secret = std::env::var("EDGE_SECRET").ok();
    let jwt_secret = std::env::var("JWT_SECRET")?;
    let jwt_expiry_hours = std::env::var("JWT_EXPIRY_HOURS")?.parse()?;

    // 2. インフラ層セットアップ
    let db_pools = DbPools::from_env().await?;
    let redis_client = redis::Client::open(redis_url)?;

    // 3. リポジトリ組み立て（CQRS + キャッシュ）
...
│                                                             │
│  ┌───────────────┐    ┌───────────────┐                    │
│  │ DbPools       │    │ Redis Client  │                    │
│  │ (Writer/Reader)│   │               │                    │
│  └───────┬───────┘    └───────┬───────┘                    │
│          │                    │                            │
│          ▼                    ▼                            │
│  ┌───────────────┐    ┌───────────────┐                    │
│  │PostgresTodo-  │    │ TodoCache     │                    │
│  │Writer/Reader  │    │               │                    │
│  └───────┬───────┘    └───────┬───────┘                    │
│          │                    │                            │
│          ▼                    │                            │
